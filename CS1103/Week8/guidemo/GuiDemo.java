...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		menuBar.add(iconSupport.createMenu()); //add a stamper menu
		setJMenuBar(menuBar);
	...
	//Add a toolbar method which is used to place icon menus at north position of the window
	private JToolBar makeToolbar() {
		...
		tbar.add(new ColorIconAction("Color..."));
		return tbar;
	}
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color..."))
    			...
    			//Added a icon for custom action
    ...
    //Add ColorIconAction to create new icon for a color menu
	private class ColorIconAction extends AbstractAction {
		ColorIconAction(String text) { 
			super(text); //inherit a text object that is given as an argument
			...
			putValue(Action.SHORT_DESCRIPTION, "Use a solid background color as the background."); 			
		}
		public void actionPerformed(ActionEvent evt) {
			Color c = JColorChooser.showDialog(drawPanel, "Select Color for Background", drawPanel.getBackground());
			if (c != null) {
